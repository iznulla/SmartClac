...
void CreditWindow::on_pushButton_cr_calc_clicked() {
  ...
  try {
    auto pay_info = controll_.CreditCalc(sum_line, precent_line, month_line,
                                         ui->comboBox->currentIndex());
    for (auto i = 0; i < pay_info.first.size(); ++i) {
      QString info = " Месяц " + QString::number(i + 1) + " || Сумма платежа " +
                     QString::number(pay_info.second[i]) +
                     " || Платеж по процентам " +
                     QString::number(pay_info.first[i]);
    ...
    auto total_info =
        controll_.ammountPercent(ui->lineEdit_cred_inp->text(), pay_info.first);
